...
static const char *logLevelString[] = {"TRACE", "DEBUG", "INFO", "WARNING", "ERROR", "FATAL"};

void log_base(LoggerLevel loggerLevel, const char* fileName, int fileLine, const char* format, ...)
{
	va_list args;

	if(configuration.level <= loggerLevel && configuration.enable == LOG_ENABLE)
	{
		printf("%s:\t%s at line %i\n\t", logLevelString[loggerLevel], fileName, fileLine);
...
void log_enable(LoggerEnable enable)
{
